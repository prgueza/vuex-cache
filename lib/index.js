  ...
  return store => {
    ...
    const RequestInterceptor = config => {
      ...
      if ((0, _isCached.default)(methods, endpoints, config)) {
        ...
        if (!reload && cachedResponse && (garbageColector || (0, _moment.default)(cachedResponse.expires).isAfter((0, _moment.default)()))) {
    ...
    const ResponseInterceptor = response => {
      ...
      if (store.getters.fallback.includes(status)) {
        ...
        if (cachedResponse) {
          ...
          response = (0, _cachedResponseBuilder.default)(config);
        }
      } else if (cache) {
