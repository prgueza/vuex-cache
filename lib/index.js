...
function cachePlugin(instance, customSettings) {
  return store => {
    ...
    const RequestInterceptor = config => {
      ...
      const garbageCollector = localgarbageCollector === undefined ? globalgarbageCollector : localgarbageCollector; // If the clear setting is set in the config, clear up cache before making the request

      if (clear) store.dispatch('clear', {
        clear
    ...
    store.registerModule('cache', {
      ...
      actions: {
        ...
        clear(_ref10, payload) {
          let state = _ref10.state,
              commit = _ref10.commit,
              getters = _ref10.getters;
          const groupNames = payload.clear; // Groups with related calls to be deleted
